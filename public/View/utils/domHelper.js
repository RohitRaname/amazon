...
export const activeel = (elementArr, target) => {
  elementArr.forEach((mov) => {
    mov.dataset.active = target === mov;
  });

};

export const unActiveAllEls= (elementArr) => {
  elementArr.forEach((mov) => {
    mov.dataset.active =false;
  });
