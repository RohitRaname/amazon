...
class CreateMultipleTweetsView extends create_multiple_post_view {
  _topParentEl = document.querySelector('.createTweets');
  ...
  get_left_space_img_arr(upload_imgs_exist, upload_imgs) {
    let upload_img_left_space = upload_imgs_exist ? 4 - upload_imgs.length : 4;
    const upload_img_left_space_arr = [];
    for (let i = 0; i < upload_img_left_space; i++)
      upload_img_left_space_arr.push(i);
    return upload_img_left_space_arr;
  }
  ...
  generate_tweet_markUp(first_tweet_item, tweet, cur_user, tweet_num) {
    ...
    if (posted_tweet) {
      posted_tweet.text = `${posted_tweet.text}`;
      posted_tweet.user.name = `${posted_tweet.user.name}`;
    }
    ...
    const upload_imgs_exist =
      upload_imgs && upload_imgs.length > 0 ? true : false;

    const left_img_space_arr = this.get_left_space_img_arr(
      upload_imgs_exist,
      upload_imgs
    );
    ...
    if (tweet_type === 'comment') {
      ...
      posted_tweet_html = ` 
         <div class="tweet postedTweet no-hover" data-comment-avatar=${
           posted_user.avatar
         }>
            <div class="tweet-container tweet-reply-content tweet-line"><img class="tweet-img img--md" src="/img/users/${
              ...
              <div class="tweet-content tweet-reply-detail">
                <div class="tweet-user__info preview-item" data-no-hover-change="">
                  <p class="tweet-user__name h-8">${posted_user.name}</p>
    ...
    if (tweet_type === 'quote') {
      quote_html = `
            <article class="quotetweet-posted-tweet no-hover p-sm" data-tweet-id=${
              posted_tweet._id
        ...
        posted_tweet.user.profilePic || 'default.png'
                ...
                <div class="tweet-content">
                    ...
                    <div class="tweet-message word-break p--md mg-sm">
                    ${
                      posted_tweet.text === 'undefined' ? '' : posted_tweet.text
                    }
                 </div>
    ...
    <form
      ...
      data-progress-line-exist="true"
      data-is_form_hidden="true" 
      data-form_submitting="false"
    >

      <div class="progress-line" data-progress="0"></div>
          ...
          <header class="createTweet-header mg-v-sl">
            <div class="form-group mg-lw">
              <textarea
                ...
                name="text"
                value=${text === 'undefined' || !text ? '' : text}
              >${text === 'undefined' || !text ? '' : text}</textarea>
            </div>
...
${left_img_space_arr
  .map(
    (el) => `<figure class="tweet-uploadImg__fig hidden ">
  <button class="btn--icon-dull pos-tl" data-action="remove-img">
  <i class="fa fa-times" aria-hidden="true"></i></button
  ><img class="tweet-uploadImg" srcalt="" />
  </figure>`
  )
  .join('')}
              </div>
  ...
  insert_tweet_in_multiple_tweets_list(
    ...
    const new_postel = this._topParentContainerEl.querySelector(
      `.createTweets-item[data-tweet-num="${tweets_present_count + 1}"]`
    );

    this._setDOMEls(new_postel);

    this._topParentEl.dataset.any_post_word_limit_exceed === 'true'
      ? this.disable_all_submit_btns()
      : this.enable_all_submit_btns();

    // set the upload files in upload_input el
    if (tweet.upload_imgs && tweet.upload_imgs.length > 0) {
      if (typeof tweet.upload_imgs[0] === 'string')
        this._uploadImgInputEl.dataset.files = JSON.stringify(
          tweet.upload_imgs
        );
      else this._uploadImgInputEl.files = tweet.upload_imgs;
    }
  }
  ...
  check_any_post_word_limit_exceed() {
    const all_post = this._getAllTweetItems();

    return all_post.some(
      (el) => el.querySelector('textarea').value.length > 200
    );
  }

  disable_all_submit_btns() {
    const all_posts = this._getAllTweetItems();
    this._topParentEl.dataset.any_post_word_limit_exceed = true;
    if (all_posts.length === 1) this._saveTweetBtn.disabled = true;
    else
  ...
  enable_all_submit_btns() {
    const all_posts = this._getAllTweetItems();
    this._topParentEl.dataset.any_post_word_limit_exceed = false;

    if (all_posts.length === 1) this._saveTweetBtn.disabled = false;
    else
      all_posts.forEach((el) => {
        el.querySelector(
          'button[data-action="save-all-tweets"]'
  ...
  enable_cur_post_submit_btn() {
    const all_post_form = this._getAllTweetItems();
    if (all_post_form.length > 1) return;
    all_post_form[0].querySelector(
      'button[data-action="save-tweet"]'
    ).disabled = false;
  }

  disable_cur_post_submit_btn() {
    const all_post_form = this._getAllTweetItems();
    if (all_post_form.length > 1) return;
    all_post_form[0].querySelector(
      'button[data-action="save-tweet"]'
  ...
  get_tweet_data(tweet, set_tweet_type_as_draft) {
    ...
    if (tweet_type === 'comment') {
      ...
      data.reply_to = posted_tweet.user.avatar;
      dataobj.reference_tweet_id = posted_tweet._id;
      data.comment_tweet_user_id = this._cur_user._id;
    }
    ...
    if (
      ...
      if (data.posted_tweet_quote_tweet_parent_id_arr === '1')
        data.posted_tweet_quote_tweet_parent_id_arr = [];
  ...
  add_handler_tweets(handle) {
    this.handleTweet(handle);
    this._hideParentWhenOverlayIsClicked();
  }
}
export default CreateMultipleTweetsView;
