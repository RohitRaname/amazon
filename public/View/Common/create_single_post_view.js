import { addClass, contains, removeClass } from '../utils/domHelper.js';
...
class ParentCreateTweetView {
  ...
  _setDOMEls(target) {
    ...
    // multiple post btn
    this._save_all_tweets_btn = this._parentEl.querySelector(
  ...
  emptyAllInputsValue(defer_img_files_remove = false) {
    ...
    this._textAreaEl.style.height = '30px';
    ...
    // we defer this input as data copies has same reference as this one
    // so when we remove this the data also get removed from copied
    ...
    this._uploadImgInputEl.dataset.files = '[]';
  ...
  _showTextWordLimitAndEnableSubmitBtn() {
    ...
    this._textAreaEl.addEventListener('input', () => {
      ...
      this.check_any_post_word_limit_exceed()
        ? this.disable_all_submit_btns()
        : this.enable_all_submit_btns();

      if (length > 0 && length < 200) {
        this._parentEl.dataset.tweetEmpty = false;
        this.enable_cur_post_submit_btn();
      } else {
        this._parentEl.dataset.tweetEmpty = true;
        this.disable_cur_post_submit_btn();
      }

      this._percentageCircleControllerEl.style.stroke =
        length > 200 ? 'red' : 'var(--color-primary-light)';
  ...
  _getAllInputsValue(parentEl) {
    ...
    data = data.reduce((acc, input) => {
      ...
      if (input.name === 'upload_image') {
        ...
        const img_set_count = Number(
          this._parentEl.querySelector('.grid--collage').dataset.items
        );

        // check the num of img set also if image set is 0 and there is file we dont need to set it
        if (input.files.length > 0) {
          acc['upload_imgs'] = input.files;

          if (img_set_count === 0) acc['upload_imgs'] = [];

          // multiple tweet case
        } else if (input.dataset.files !== '') {
          acc['set_draft_upload_img'] = JSON.parse(input.dataset.files);
          if (img_set_count === 0) acc['upload_imgs'] = [];
        }
      }
      // acc[input.name] =input.value
  ...
  getTweetDataAsFormData(dataobj) {
    ...
    if (data.upload_imgs) data.upload_imgs = [...data.upload_imgs].slice(0, 4);
